...
const Index = () => {
    ...
    let getBrands = async () => {
        setLoading(true);
        const headers = { 'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im5vYS52YW5kZW5iZXJnaGVAYXAuYmUiLCJpYXQiOjE3MzI1NDEwNTd9.G8AAh6Al4oenOLvXVbV4DtnAce00c1fwJQDs9k5p1B8' }; let response = await fetch('https://sampleapis.assimilate.be/car/models', { headers });
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View className="bg-white p-4 mb-4 rounded-lg shadow-md">
                        ...
                        <Text className="text-gray-700 mb-1">Fuel Type: {item.fuel_type}</Text>
                        <Text className="text-gray-700 mb-1">Transmission: {item.transmission}</Text>
                        <Text className="text-gray-700 mb-1">Top Speed: {item.top_speed_kmh} km/h</Text>
                        <Text className="text-gray-700 mb-1">0-100 km/h: {item.acceleration_0_to_100_kmh} sec</Text>
                        <Text className="text-gray-700 mb-1">Horsepower: {item.horsepower} hp</Text>
                        <Text className="text-gray-700 mb-1">Seating Capacity: {item.seating_capacity}</Text>
                    </View>
                )}
